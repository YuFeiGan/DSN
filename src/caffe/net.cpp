...
void Net<Dtype>::GetLearningRateAndWeightDecay() {
  ...
  for (int i = 0; i < layers_.size(); ++i) {
    ...
    //push the layerwise learning rate policy
    if (layers_[i]->layer_param().blobs_lr_policy_size()) {
      CHECK_EQ(layers_[i]->layer_param().blobs_lr_policy_size(), layer_blobs.size());
      for (int j = 0; j < layer_blobs.size(); ++j) {
        string local_lr_policy = layers_[i]->layer_param().blobs_lr_policy(j);
        params_lr_policy_.push_back(local_lr_policy);
      }
    } else {
      for (int j = 0; j < layer_blobs.size(); ++j) {
        params_lr_policy_.push_back("nothing");
      }
    }
